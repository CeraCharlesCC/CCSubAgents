...
const defaultDaemonTCPAddr = "127.0.0.1:19131"
...
func NewDefaultClient(stateDir, token string) *Client {
	if runtime.GOOS == "windows" {
		return NewHTTPClient("http://"+defaultDaemonTCPAddr, token)
	}
	return NewUnixSocketClient(filepath.Join(stateDir, "daemon", "ccsubagentsd.sock"), token)
}
...
func (c *Client) SaveText(ctx context.Context, req SaveTextRequest) (artifacts.ArtifactVersion, error) {
	var out struct {
...
func (c *Client) SaveBlob(ctx context.Context, req SaveBlobRequest) (artifacts.ArtifactVersion, error) {
	var out struct {
		Artifact artifacts.ArtifactVersion `json:"artifact"`
	}
...
func (c *Client) do(ctx context.Context, method, path string, reqBody any, out any) error {
	...
	if err := json.NewDecoder(io.LimitReader(resp.Body, DefaultMaxRequestBytes)).Decode(&env); err != nil {

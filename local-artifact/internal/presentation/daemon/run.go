...
func Run(ctx context.Context, cfg RunConfig) error {
	...
	if err != nil {
		...
		writef(cfg.Stderr, "ccsubagentsd api already active on unix://%s; starting web-only mode\n", alreadyErr.socket)
	}
	...
	if strings.TrimSpace(cfg.WebAddr) != "" {
		...
		defer func() {
			if closeErr := webServer.Close(); closeErr != nil {
				_ = closeErr
			}
		}()
		...
		writef(cfg.Stderr, "ccsubagentsd web listening on http://%s\n", cfg.WebAddr)
		if token == "" {
			writef(cfg.Stderr, "ccsubagentsd web bootstrap: open http://%s/\n", cfg.WebAddr)
		} else {
			writeln(cfg.Stderr, webBootstrapHint(cfg.WebAddr, cfg.StateDir))
		}
	}
